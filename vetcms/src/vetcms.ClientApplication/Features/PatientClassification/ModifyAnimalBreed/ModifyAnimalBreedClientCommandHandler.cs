...
{
    internal class ModifyAnimalBreedClientCommandHandler(IMediator mediator, IMapper mapper, IDialogService dialogService) : IRequestHandler<ModifyAnimalBreedClientCommand, bool>
    {
        public async Task<bool> Handle(ModifyAnimalBreedClientCommand request, CancellationToken cancellationToken)
        {
            ModifyAnimalBreedApiCommand command = mapper.Map<ModifyAnimalBreedApiCommand>(request);
            ModifyAnimalBreedApiCommandResult response = await mediator.Send(command);
            if (response.Success)
            {
